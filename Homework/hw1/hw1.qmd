...
## 8.2 Create interactive map with all crime points

**Note:**
- If this map is not properly displayed, please grade on the next map.
- This map is asking Claude Sonnet 4.5 for assitance to generate because my skills are limited to include all the data points when Quarto is rendering in a folium map with individual crime points. Therefore, I asked AI for help to save the map to a external HTML file and embed it in the Quarto document.

```{python}
#| label: create-all-crime-points-map-individual-points
#| output: false

# Set up the map centered on Toronto
toronto_center = [assault_map["lat"].mean(), assault_map["lon"].mean()]
...
# Add ALL crime points using MarkerCluster for efficient rendering
print(f"Adding all {len(assault_map):,} crime locations to map with clustering...")
...
for idx, row in assault_map.iterrows():
    color = season_colors.get(row["season"], "gray")
    
    # Minimal popup for performance
    popup_html = f"""
    <b>{row['neighbourhood']}</b><br>
    {int(row['occurrence_year'])}-{int(row['occurrence_month']):02d}-{int(row['occurrence_day']):02d} {int(row['hour']):02d}:00<br>
    <span style="color: {color};">●</span> {row['season']} ({row['time_period']})
    """
...
<div style="position: fixed; 
    ...
    <h3 style="margin: 0; font-size: 18px; color: #2c3e50; font-family: Arial;">
        Assault Crime Locations in Toronto (2024)
    </h3>
</div>
"""
m.get_root().html.add_child(folium.Element(title_html))

# Add legend for seasons with note
legend_html = """
<div style="position: fixed; 
    ...
    <p style="margin: 0 0 8px 0; font-weight: bold; font-size: 14px; color: #2c3e50;">Season</p>
    <p style="margin: 5px 0;"><span style="color: #e74c3c; font-size: 18px;">●</span> Summer</p>
    <p style="margin: 5px 0;"><span style="color: #f39c12; font-size: 18px;">●</span> Fall</p>
    <p style="margin: 5px 0;"><span style="color: #2ecc71; font-size: 18px;">●</span> Spring</p>
    <p style="margin: 5px 0;"><span style="color: #3498db; font-size: 18px;">●</span> Winter</p>
    <hr style="margin: 10px 0; border: 1px solid #bdc3c7;">
    <p style="margin: 0; font-size: 11px; color: #7f8c8d; line-height: 1.4;">
        <i>Note: When zoomed out, cluster colors reflect the dominant season among grouped points.</i>
    </p>
</div>
"""
m.get_root().html.add_child(folium.Element(legend_html))

# Save to external HTML file to bypass Quarto's rendering limitations
map_file = "crime_map_2024.html"
m.save(map_file)
print(f"Map saved to {map_file} with {len(assault_map):,} points")
print(f"Map will be displayed below via iframe")
```

<iframe src="crime_map_2024.html" width="100%" height="600px" style="border:1px solid #ccc; border-radius:8px;"></iframe>

## 8.3 Neighbourhood aggregation map

```{python}
#| label: create-all-crime-points-map-neighbourhood-aggregation
