...
print(f"Most frequent OFFENCE within MCI_CATEGORY {top_category}:", top_offence)
```
...
print(f"\nNumber of records with (0,0) coordinates: {zero_coords.sum()}")
print("\nSample of (0,0) coordinate records:")
display(crime.loc[zero_coords, ["neighbourhood", "lat", "lon"]].head(10))

# Remove rows with (0,0) coordinates
crime = crime.loc[~zero_coords].copy()
print(f"\nRecords after removing (0,0) coordinates: {len(crime):,}")
...
```
```{python}
#| label: identify-remaining-nsa-records

# Clean impossible data
# - Identify remaining NSA records
nsa_mask = crime["neighbourhood"].eq("NSA")
print(f"Number of records with neighbourhood == 'NSA': {int(nsa_mask.sum()):,}")

print("\nSample of NSA records:")
display(crime.loc[nsa_mask, ["neighbourhood", "lat", "lon"]].head(10))

# Remove NSA records
n_before = len(crime)
crime = crime.loc[~nsa_mask].copy()
print(f"\nRemoved {n_before - len(crime):,} NSA records. Remaining records: {len(crime):,}")

# Sanity check: are remaining coordinates within the GTA? (approximate bounding box)
gta_lat_min, gta_lat_max = 43.20, 44.30
gta_lon_min, gta_lon_max = -80.20, -78.60

missing_coord_mask = crime[["lon", "lat"]].isna().any(axis=1)
in_gta_mask = (
    ~missing_coord_mask
    & crime["lat"].between(gta_lat_min, gta_lat_max)
    & crime["lon"].between(gta_lon_min, gta_lon_max)
)
out_of_gta_mask = (~missing_coord_mask) & (~in_gta_mask)

print(f"\nMissing coordinates after cleaning: {int(missing_coord_mask.sum()):,}")
print(f"Coordinates outside GTA bounding box: {int(out_of_gta_mask.sum()):,}")
```
## 2.5 Summary
...
- I filtered the data to only the selected category/offence and kept only the “necessary columns” from Question 1 (`key_vars`).
...
- I identified records with coordinates **(lon, lat) = (0, 0)** and treated these as invalid geographic coordinates (documentation indicates “not specified area/outside Toronto”, our primary question is focused on Toronto only). Then I removed these rows from the dataset.
- I identified records with neighbourhood **NSA** that is non (0,0) coordinates and treated these as invalid geographic coordinates (documentation indicates “not specified area/outside Toronto”, our primary question is focused on Toronto only). There were 170 such rows in total. Then I removed these rows from the dataset.
- I performed a sanity check to ensure that the remaining coordinates are within the GTA bounding box (approximate bounding box). There were no coordinates outside the GTA bounding box.
- The resulting analytic dataset has **163,490 rows** and **11 columns**.
...
- I cross-referenced the data with the data documentation Appendix A: Open Data Summary Table, and found that the valid time range for **Assault** crimes data is from 2014 to 2025.09.30.
...
**Interpretation:** For each 1-year increase from 2014 to 2024, the expected annual assault crime count increases by approximately 3.21%, suggesting a statistically significant (p-value < 0.05) upward trend in assault crimes over this period.
...
The Poisson model captures the overall upward trend, though the residual plot shows systematic deviations (notably around 2021). The dispersion parameter (≈69.77) indicates **overdispersion** as it is significantly greater than the threshold 1.5, violating the Poisson assumption (variance = mean) and making standard errors unreliable.

## 4.5 Summary

Yes, there is statistically significant evidence of an upward trend in assault crimes across Toronto from 2014 to 2024, with the Poisson regression estimating approximately a **3.21% annual increase** in expected crime counts. However, the model exhibits severe overdispersion, suggesting that while the overall increasing trend is present, the simple Poisson model may underestimate the uncertainty in this estimate， which means the model results may not be reliable.
...
**Interpretation:** The chi-square test shows strong evidence against the null hypothesis of uniform distribution (χ² = 38.26, p < 0.05), indicating significant seasonal variation in assault crimes in 2024. Note, with standardized residuals exceeding the threshold of $|2|$, it is evident that **Summer** has notably **higher** counts than expected, while **Winter** has **lower** counts than expected.
...
Therefore, **Summer** looks most different, with **fewer Day** crimes than expected and **more Night** crimes than expected (standardized residuals: Day = −2.47, Night = +2.51).

# Step 7. Look at neighbourhood patterns

## 7.1 Summarize crime counts by neighbourhood

```{python}
#| label: neighbourhood-stats-summary
...
display(neighbourhood_summary.nlargest(10, "total_count").reset_index(drop=True))
```
...
## 8.1 Prepare data for mapping

```{python}
#| label: prepare-map-data

# Use 2024 data with valid coordinates
assault_map = assault_recent.copy()

print(f"Records with valid coordinates for mapping: {len(assault_map):,}")
```

## 8.2 Neighbourhood aggregation map

```{python}
#| label: create-neighbourhood-aggregation-map

# Aggregate by neighbourhood - count crimes and calculate mean location
...
This neighbourhood-aggregated map shows strong spatial clustering of assaults, with the highest counts concentrated in downtown Toronto (larger orange markers).  
Most neighbourhoods farther from the core have smaller, lower-count markers, indicating a more diffuse distribution outside the centre.  

Marker **size and color** are proportional to the total number of assaults in each neighbourhood (marker location is the neighbourhood crime record locations' mean in terms of latitude/longitude), so the map emphasizes area-level concentration rather than exact incident locations.
