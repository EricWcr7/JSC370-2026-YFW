...
# Handling missing values at the country-level:
...
- **Alcohol (`x1`):** $\hat{\beta} = 0.592$ ($p < 0.05$, therefore statistically significant). While holding other variables constant, a one litre increase in annual alcohol consumption per capita leads to an increase of approximately 0.592 years on average in expected life expectancy.
- **GDP per capita (`x2`):** $\hat{\beta} = 0.0002$ ($p  < 0.05$, therefore statistically significant). While holding other variables constant, a 1 current US dollar increase in GDP per capita leads to an approximately 0.0002 years increase on average in expected life expectancy.
- **Year (categorical, reference level = 1996):** Comparing to 1996, life expectancy is higher by about 3.44 years in 2016 ($p<0.05$, therefore statistically significant) and by about 4.09 years in 2019 ($p < 0.05$, therefore statistically significant), **holding alcohol consumption and GDP per capita constant**.
- **Overall Fit:** The adjusted $R^2$ is 0.483, indicating the linear model explains about 48.3% of the variance.

GAM (Non-linear) Estimates:

- **Alcohol (`s(0)`):** The smooth term for alcohol has an EDoF of 12.2 but a p-value of 0.85 which is greater than 0.05 (the threshold for statistical significance), suggesting that after adjusting for the non-linear effect of GDP and the categorical year effect, the relationship between alcohol and life expectancy is not statistically significant in this flexible model.
- **GDP per capita (`s(1)`):** The smooth term for GDP has an EDoF of 8.8 and is highly statistically significant ($p < 0.05$). The EDoF > 1 indicates a strong non-linear relationship (likely diminishing returns where life expectancy rises with GDP but plateaus at higher levels).
- **Year (`f(2)`):** The year factor has EDoF 1.9 with ($p = 0.0669 > 0.05$), which is marginal evidence at the 0.05 level (suggestive differences by year, but weaker once non-linear GDP is modeled).
- **Overall Fit:** The Pseudo $R^2$ is 0.712, which is substantially higher than the linear model, suggesting that incorporating non-linearity provides a much better fit to the data.
...
spec = [
    (0, 0, "Annual Pure Alcohol Consumption (litres per capita)", "blue", "Effect of Alcohol"),
...
- **Alcohol:** The plot for alcohol consumption shows a line that wiggles around zero with wide confidence intervals. This plot confirms the not statistically significant p-value ($p \approx 0.85 > 0.05$) from the model summary, indicating that once we account for GDP and year, alcohol consumption does not have a statistically significant non-linear association with life expectancy in this dataset.

- **GDP per Capita:** The plot for GDP shows a distinct non-linear pattern: a steep positive slope at lower income levels (below approximately $20,000). Then it flattens out significantly as GDP increases. This reflects GDP per capita has a significant impact on health for poorer/developing countries, but adding more GDP per capita to already rich/developed countries yields smaller and smaller gains in life expectancy.
