...
# Center year to get a more interpretable year coefficient
q5_df["year_c"] = q5_df["year"] - q5_df["year"].mean()

y = q5_df["life_expectancy"].values
X_linear = q5_df[["alcohol_litres_per_capita_15plus", "gdp_per_capita", "year_c"]].values
...
X_gam = q5_df[["alcohol_litres_per_capita_15plus", "gdp_per_capita", "year_c"]].values
gam_mod_5 = LinearGAM(
    s(0, n_splines=20) +  # smooth alcohol
    s(1, n_splines=20) +  # smooth GDP per capita
    l(2)                  # linear year adjustment since it only has three possible values (1996, 2016, 2019) before centering
).fit(X_gam, y)
...
- **Alcohol (`x1`):** $\hat{\beta} = 0.590$ ($p < 0.001 < 0.05$, therefore statistically significant). While holding other variables constant, a one litre increase in annual alcohol consumption per capita leads to an increase of approximately 0.59 years on average in expected life expectancy.
- **GDP per capita (`x2`):** $\hat{\beta} = 0.0002$ ($p < 0.001 < 0.05$, therefore statistically significant). While holding other variables constant, a 1 current US dollar increase in GDP per capita leads to an approximately 0.0002 years increase on average in expected life expectancy.
- **Year (`x3`, centered):** $\hat{\beta} = 0.176$ ($p < 0.001 < 0.05$, therefore statistically significant). While holding other variables constant, each additional year is associated with an approximately 0.18 years increase on average in expected life expectancy.
- **Overall Fit:** The adjusted $R^2$ is 0.485, indicating the linear model explains about 48.5% of the variance.

GAM (Non-linear) Estimates:

- **Alcohol (`s(0)`):** The smooth term for alcohol has an EDoF of 12.2 but a p-value of 0.848 which is greater than 0.05 (the threshold for statistical significance), suggesting that after adjusting for the non-linear effect of GDP and linear effect of year, the relationship between alcohol and life expectancy is not statistically significant in this flexible model.
- **GDP per capita (`s(1)`):** The smooth term for GDP has an EDoF of 8.8 and is highly statistically significant ($p < 0.001 < 0.05$). The EDoF > 1 indicates a strong non-linear relationship (likely diminishing returns where life expectancy rises with GDP but plateaus at higher levels).
