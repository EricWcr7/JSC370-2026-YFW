...
According to the confusion matrix, **Private for-profit** is the easiest category to predict by the pruned tree because the model correctly identified `{python} cm[class_labels.index("Private for-profit"), class_labels.index("Private for-profit")]` out of `{python} cm[class_labels.index("Private for-profit"), :].sum()` workers, which is `{python} round(cm[class_labels.index("Private for-profit"), class_labels.index("Private for-profit")] / cm[class_labels.index("Private for-profit"), :].sum() * 100, 2)`%. However, **Unemployed**, **Self-employed**, and **Government** are the hardest categories to predict because the pruned tree cannot correctly identify any of the workers in these categories.
